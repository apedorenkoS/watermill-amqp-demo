...
import (
	...
	"github.com/apedorenkoS/watermill-amqp-demo/random-single-and-one-everything/all-consumer/internal/config"
	"github.com/apedorenkoS/watermill-amqp-demo/random-single-and-one-everything/all-consumer/internal/event"
...
func main() {
	...
	transport, err := event.NewTransport(conf, "receive.everything.queue", "#")
	if err != nil {
		log.Panic().Msgf("Failed to create transport: %v", err)
	}

	err = transport.TopicSubscribe(event.TopicExchange, processEvent)
	if err != nil {
